...
Wie schon für die letzten beiden Sitzungen, können Sie das komplette R-Skript [`r fontawesome::fa("download")` hier herunterladen](https://raw.githubusercontent.com/martscht/PsyMSc1/master/inst/scripts/cfa.R).
...
<img src="images/cfa1.png" width="33%"/>
...
wobei $s$ die Anzahl bekannter Informationen und $p$ die Anzahl von manifesten Variablen im Modell ist. In unserem Fall also
...
<img src="images/cfa2.png" width="33%"/>
...
Mit `1*` legen wir den konkreten Wert der Faktorladung fest. Analog hierzu könnten wir mit `2*` diese Ladung auf den Wert 2 fixieren. Das gleiche Prinzip gilt in `lavaan` für alle Modellparameter. Probieren Sie unten aus, das Modell so zu schreiben, dass die Faktorladungen frei sind, aber die latente Varianz auf 1 fixiert ist! *Bitte lassen Sie sich am Ende das Modell auch mit `mod1b` ausgeben, weil sonst in diesem Dokument eine Warnung (bezüglich `?invisible`) entsteht.*
...
grade_result(
  fail_if(~ (!grepl('~~1*Q', gsub(' ', '', .result))), 'Die Residualvarianzen brauchen Sie nicht zu fixieren. Diese können ruhig geschätzt werden.'),
...
Was hier erneut fehlt ist die Mittelwertsstruktur, die nicht per Voreinstellung mit in das Modell aufgenommen wird. Diese können wir aber per Argument von `cfa` anfordern:
...
<img src="images/cfa3.png" width="66%"/>
...
Nutzen Sie die Befehle, die wir im Abschnitt zum einfaktoriellen Modell kennengelernt haben, um sich über die Ergebnisse dieses zweifaktoriellen Modells einen Überblick zu verschaffen!
...
```{r two_factors_summary, exercise = TRUE, exercise.setup = 'two_factors_prep', exercise.eval = FALSE}
...
Die `summary` enthält nun den neuen Abschnitt `Covariances` der, wie Sie vermutlich erraten können, Kovarianzen zwischen den latenten Variablen enthält. Versuchen Sie mit `inspect` die **Korrelation** zwischen den beiden Faktoren in Erfahrung zu bringen!

```{r two_factors, exercise = TRUE, exercise.setup = 'two_factors_prep', exercise.eval = FALSE}

```

```{r two_factors-hint-1}
...
```{r para-hint-4}
# Der lavaan Operator für Mittelwerte und Interceps ist '~1'
```
...
```{r extended_summary, exercise = TRUE, exercise.setup = 'two_factors_prep', exercise.eval = FALSE}
summary(fit_two)
```
...
Um jetzt z.B. die LogLikelihood aus diesem Objekt zu ziehen, können wir mit diese über eckige Klammern auswählen. Dazu können wir erst einmal gucken, welche Werte in dem Objekt abgespeichert sind:
...
```{r cfi, exercise = TRUE, exercise.setup = 'cfi_prep', exercise.eval = FALSE}

```

```{r cfi-hint-1}
# Namen aller Fitstatistiken im Objekt
names(modelfit)
```
...
Die einzelnen Einträge können wir jetzt quasi als $z$-Werte interpretieren: wenn ein Wert vom Betrag also größer als 1.96 ist, ist der Unterschied zwischen Modellimplikation und Empirie an dieser Stelle auf einem $\alpha$-Fehlerniveau von 5% statistisch bedeutsam. Wir sehen z.B. dass in unserem Modell der Zusammenhang zwischen Q10 und Q15 sehr stark unterschätzt wird (das Residuum ist hier `r round(residuals(fit_two, 'standardized')$cov[6,5], 2)`). Eine detaillierte Untersuchung des lokalen Misfits anhand der Residuen in der Kovarianzmatrix finden Sie im [im Anhang](#Residuals).
...
<img src="images/cfa4.png" width="66%"/>
...
Wenn Sie möchten, können Sie sich die Modellergebnisse hier im Detail angucken und sehen, was sich [gegenüber dem Zweifaktormodell](#TwoFactors) verändert hat:

```{r resi_three, exercise = TRUE, exercise.setup = 'crossloading_prep', exercise.lines = 5}
...
kable(fitcomp, digits = c(1, 3, 0, 3, 3, 3, 3, 3) , align = 'c')
```
...
```{r resi_cov, exercise = TRUE, exercise.setup = 'two_factors_prep', exercise.eval = FALSE}
mod_four <- ' ... '
fit_four <- cfa(...)
...
```{r resi_cov-hint-1}
# Kovarianzen werden in lavaan mit ~~ angesprochen
...
```{r resi_cov-hint-2}
# Vergessen Sie nicht auch die Mittelwertsstruktur bei der Schätzung anzufordern
...
## Appendix A: Lokalisierung von Misfit in Kovarianzmatrizen {#Residuals}
...
Probieren Sie es doch am Besten selbst mal mit den Items aus, die als Manifestationen der latenten Variable *control of information* dargestellt werden. An welchen Stellen springt Ihnen hier Misfit ins Auge? Wenn Sie die [Itemformulierungen](#TwoFactors) betrachten - woran könnte das inhaltlich liegen?
...
```{r resi, exercise = TRUE, exercise.setup = 'resi_prep'}
...
## Appendix B: Hierarchie der Messmodelle {#AppendixA}
...
<img src="images/mess1.png" width="66%"/>
...
Probieren Sie einfach mal selbst aus das essentielle $\tau$-parallele Messmodell aufzustellen und zu prüfen! (Nennen Sie es am besten `mod4` und das Ergebnisobjekt `fit4`.) Zur Erinnerung hier noch einmal die Hierarchie der Messmodelle:

<img src="images/mess2.png" width="66%"/>

In den Tipps finden Sie weitreichende Hinweise. Der letzte Tipp (Nummer 5) zeigt Ihnen dann die vollständige Lösung.

```{r esspar_prep}
mod1 <- 'stat =~ test1 + test2 + test3'
fit1 <- cfa(mod1, stat_test, meanstructure = TRUE)

mod2 <- 'stat =~ 1*test1 + 1*test2 + 1*test3'
fit2 <- cfa(mod2, stat_test, meanstructure = TRUE)

mod3 <- 'stat =~ 1*test1 + 1*test2 + 1*test3
  test1 ~ (alp)*1
  test2 ~ (alp)*1
  test3 ~ (alp)*1'
fit3 <- cfa(mod3, stat_test, meanstructure = TRUE)
```

```{r esspar, exercise = TRUE, exercise.eval = FALSE, exercise.lines = 15, exercise.setup = 'esspar_prep'}

```

```{r esspar-hint-1}
# Im Modell Residualvarianzen ansprechen
...
```{r esspar-hint-2}
# Residualvarianzen müssen mit einem Label gleichgesetzt werden
```

```{r esspar-hint-3}
# Damit das Modell mit den bisherigen Modellen verglichen
...
```{r esspar-hint-4}
# Das Modell muss mit dem essentiell tau-äquivalenten,
...
Das $\tau$-parallele Messmodell ist das restriktivste Messmodell. Hier sind Ladungen, Intercepts und Residualvarianzen über die drei Tests hinweg gleich gesetzt. Das bedeutet, dass zusätzlich zu den beim $\tau$-äquivalenten Messmodell besprochenen Eigenschaften noch dazu kommt, dass alle Varianzen gleich sind, sodass die Reliabilität der verschiedenen Tests ebenfalls identisch ist.
...
```{r compi, exercise = TRUE, exercise.setup = 'compi_prep'}
