...
<mark style="background-color: orange"> Den Unterpunkten `Regressions` entnehmen wir, dass die Pfadkoeffizienten  recht ähnlich groß zu sein scheinen. Die Standardfehler überlappen sich stark, wenn wir `Estimate` $\pm$ `Std.Err` für einen Koeffizienten über die Gruppen hinweg rechnen. Schauen wir uns die Interzepte in den Unterpunkten `Intercepts` über die Gruppen an, erkennen wir an der Effektkodierung für `ZD` und `BOEE`, dass die latenten Interzepte ($\kappa$) in der einen Gruppe auf 0 und in der anderen Gruppe frei geschätzt wurde. Der Interzept in Gruppe 2 (den Männern) von Zeitdruck ist signifikant von 0 verschieden, das von Emotionaler Erschöpfung nicht. Dies bedeutet, dass sich Männern und Frauen in ihrem latenten Mittelwert von Zeitdruck unterscheiden. Das negative Vorzeichen verrät uns, dass Männer im durchschnitt weniger Zeitdruck erleben. Dieser signifikante Unterschied könnte der Grund gewesen sein, dass die vollständige Invarianz verworfen wird. Um dies genauer zu prüfen, müssten wir sukzessive alle Parameter über die Gruppen gleichsetzen und schauen, für welchen diese Gleichsetzung zu einer signifikanten Verschlechterung des Modells führt. </mark>
