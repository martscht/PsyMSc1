...
Auch sind Unterschiede in den Reliabilitäten auf Unterschieden in den Varianzen der latenten Variablen zurückzuführen, denn die Formel für die Reliabilität ist (nach dieser Formel würde auch `lavaan` für uns die Reliabilitäten/$R^2$ berechnen):
$$\text{Rel}\left[X^{(g)}\right]=\frac{\lambda^2\phi^{(g)}}{\lambda^2\phi^{(g)}+\theta}=\frac{\lambda^2\phi^{(g)}}{\mathbb{V}ar\left[X^{(g)}\right]}.$$ 
Hier ist also auch nur noch $\phi$ variabel über die Gruppen und somit allein verantwortlich für Unterschiede in den Reliabilitäten über die Gruppen. Für standardisierte $X$ ($\mathbb{V}ar\left[X\right]=1$) und Skalierer auf der latenten Varianz ($\phi=1$) vereinfacht sich diese Formel zu $\text{Rel}(X)=\lambda^2$, weswegen wir in diesem Fall die Reliabilität ganz einfach über die Faktorladungen erhalten. Dies gilt im Allgemeinen jedoch nicht! Wir schätzen das strikt-invariante Modell durch `group.equal = c("loadings", "intercepts", "residuals")`.

Gehen wir von **vollständiger Invarianz** aus, so sind alle Parameter über die Gruppen hinweg gleich. Es gibt also keinerlei Unterschiede mehr zwischen den Gruppen. Die Variablen selbst bleiben Beobachtungen, die individualspezifisch bleiben, weswegen sie in der Grafik weiterhin in Farbe dargestellt werden:
...
Die folgende Tabelle dient der Übersicht der Invarianzstufen sowie ihrer Implikationen:

```{r, echo = F}
library(knitr)
library(kableExtra)
df <- data.frame(Invarianzstufe = c("konfigural", "metrisch/schwach", "skalar/stark", "strikt", "vollständig"), Annahme = c("gleiche Modellstruktur", "gleiche Modellstruktur und Faktorladungen", "gleiche Modellstruktur, Faktorladungen und Interzepte", "gleiche Modellstruktur, Faktorladungen, Interzepte und Residualvarianzen", "gleiche Modellstruktur und alle Modellparameter sind identisch"), Implikation = c("gleichen Beziehungen zwischen Variablen", "latenten Variablen haben gleiche Bedeutung; wahre Anteile vergleichbar, durch Restriktion latenten Varianzen vergleichbar", "Unterschiede in den beobachteten Mittelwerten lassen sich auf Unterschiede in den latenten Mittelwerten zurückführen, Gleichheit latenter Mittelwerte testbar durch Restriktion", "Unterschiede in beobachteten Varianzen oder Reliabilitäten zurückzuführen auf unterschiede in den latenten Varianzen", "keine Unterschiede zwischen den Gruppen"))
kable(df,"latex", booktabs = T) %>% column_spec(1, bold = T) %>% column_spec(2, width = "15em") %>% column_spec(3, width = "15em")  %>%  row_spec(0, bold = T)
```
