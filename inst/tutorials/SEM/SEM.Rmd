...
<mark style="background-color: orange"> Mit Invarianz meinen wir die Gleichheit von Parametern über Gruppen hinweg. Welche Stufen der Invarianz es gibt, was diese bedeuten und wie wir diese spezifizieren gucken wir uns im Folgenden an.  So wie wir die indirekten Effekte bestimmt und die Koeffizienten für beide Gruppen benannt haben, lassen sich auch Invarianzen händisch prüfen.Wenn zwei Koeffizienten das selbe Label tragen, werden diese Parameter in den Gruppen auf den gleichen Wert gesetzt. Wir könnten nun für die jeweiligen Invarianzstufen die Parameter händisch gleichsetzen. Dieses ganze Prozedere erscheint recht aufwendig. Allerdings kann so in jedem Schritt überprüft werden, dass die Parameter richtig restringiert wurden. Auch lassen sich so auch leicht partielle Invarianz einbauen, in welchen bspw. nicht alle Faktorladungen über die Gruppen hinweg gleich sind. Außerdem könnten Invarianzen nur für bestimmte Variablen angenommen werden. Glücklicherweise enthält das `lavaan`-Paket die extrem hilfreiche Funktion, die automatisiert durchführt. Dazu müssen wir lediglich in der Schätzung unserer Modelle in `sem` das Zusatzargument `group.equal` spezifizieren. Bevor wir mit den Analysen beginnen, wollen wir die Invarianzstufen noch einmal wiederholen.

#### Invarianzstufen
</mark> Die Invarianzstufen sind nach Einschränkungen der Modellparameter sortiert und werden auch (fast) immer in dieser Reihenfolge sukzessive getestet: Die erste und am wenigsten restriktive Invarianzstufe ist die **konfigurale Invarianz**. Unter konfiguraler Invarianz verstehen wir die Gleichheit der Modelle in den Gruppen - oder salopp gesprochen: gilt konfigurale Invarianz, dann gelten in beiden Gruppen die selbe Faktorstruktur; *hier also*: hängt die Faktorstruktur nicht vom Geschlecht ab. Wie müssen wir nun`group.equal` im `sem`-Befehl spezifizieren, um konfigurale Invarianz zu prüfen? Lassen wir es ganz weg oder setzten es auf `group.equal = c("")`, so landen wir beim gewünschten Ergebnis (gleiche Modellstruktur wird über die Gruppen hinweg angenommen). Um die weiteren Invarianzstufen und deren Implikationen besser zu verstehen, schauen wir uns noch einmal die modellimplizierte Varianz und die modellimplizierte Erwartung (den modell implizierten Mittelwert) einer manifesten Variable $X$ an. Wir verwenden $^{(g)}$ um Unterschiede zwischen Gruppen darzustellen. Lassen wir dies weg, so gehen wir davon aus, dass der Parameter gleich ist über die Gruppen hinweg. Alle anderen Indizes lassen wir weg, um das ganze übersichtlicher zu gestalten.

Eine einfache modellimplizierte Gleichung einer Messung $X$ ist:

$$X^{(g)}=\tau^{(g)}+\lambda^{(g)}\xi^{(g)}+\delta^{(g)}.$$
$\tau$ ist das Interzept und $\lambda$ ist die Faktorladungen. Die Varianz von $X$ ist folglich:
$$\mathbb{V}ar\left[X^{(g)}\right]=\left(\lambda^{(g)}\right)^2\phi^{(g)}+\theta^{(g)}.$$
$\phi$ ist die Varianz der latenten Variable ($\xi$) und $\theta$ ist die Residualvarianz, also die Varianz von $\delta$. Der Mittelwert von $X$ ergibt sich als:
$$\mathbb{E}\left[X^{(g)}\right]=\tau^{(g)}+\lambda^{(g)}\kappa^{(g)},$$
wobei $\kappa$ das Interzept (den Mittelwert) von $\xi$ darstellt. Da $X$ von so vielen Parametern abhängt, ist ohne weitere Annahmen nicht zu sagen, ob Unterschiede zwischen Gruppen bspw. im Mittelwert von $X$ auf Unterschiede in der latenten Variable oder auf Unterschiede in der Messung zurückzuführen sind. Ähnlich sieht es für die Varianz aus.

Die nächste Invarianzstufe ist die **metrische** oder **schwache Invarianz**. Nehmen wir metrische Invarianz an, so gehen wir davon aus, dass die Faktorladungen ($\lambda$) über die Gruppen hinweg gleich sind. Dies impliziert, dass die latenten Variablen über die Gruppen hinweg die gleiche Bedeutung haben. Die latenten Variablen lassen sich also inhaltlich gleich interpretieren. Dies ist sehr wichtig, wenn wir bspw. Fragebögen oder Tests über Gruppen (z.B. Länder oder das Geschlecht) hinweg vergleichen wollen. Die Varianz von $X$ vereinfacht sich zu (weglassen von $^{(g)}$ symbolisiert Gleichheit über die Gruppen hinweg):
$$\mathbb{V}ar\left[X^{(g)}\right]=\lambda^2\phi^{(g)}+\theta^{(g)}.$$
Hierbei ist es so, dass $\lambda^2\phi^{(g)}$ gerade die Systematik oder den wahren Anteil in der Varianz von $X$ beschreibt. Falls metrische Invarianz gilt, so lassen sich also Unterschiede in den wahren Anteilen der Varianzen der Beobachtungen auf Unterschiede in den Varianzen zurückführen (Achtung: dies ist nicht das gleiche wie Gleichheit der Reliabilitäten, was anteilig die Systematik in $X$ an der gesamten Varianz von $X$ beschreibt - hier für müssten ebenfalls $\theta$ gleich sein). Dies bedeutet,  wenn wir metrische Invarianz nicht verwerfen (also weiter davon ausgehen, dass metrische Invarianz gilt), dass wir durch weitere Modellrestriktionen die latenten Varianzen auf Gleichheit prüfen können. Dies ist nicht zulässig, wenn metrische Invarianz *nicht* gilt, also die Faktorladungen sich über die Gruppen hinweg unterscheiden. Spezifizieren wir `group.equal = c("loadings")` so wird das metrisch-invariante Modell geschätzt. 


Unter **skalarer** oder **starker Invarianz** verstehen wir die nächste Invarianzstufe, welche zusätzlich zu den Faktorladungen auch noch die Interzepte ($\tau$) als invariant über die Gruppen hinweg annimmt. Sind die Interzepte der Items über die Gruppen hinweg gleich, so bedeutet dies, dass die Itemschwierigkeit (also die Durchschnittliche Antworttendenz) über die Gruppen hinweg gleich ist. Die Itemschwierigkeit lässt sich beispielsweise so berechnen: $\frac{\bar{X}-min(X)}{max(X)-min(X)}$, es wird also der Mittelwert im Verhältnis zur Itemrange gesetzt. Liegt die Itemschwierigkeit bei 100% so kreuzen alle Personen das Maximum an - aus diesem Grund wird auch manchmal von Itemleichtigkeit gesprochen. Der Mittelwert von $X$ vereinfacht sich zu:
$$\mathbb{E}\left[X^{(g)}\right]=\tau+\lambda\kappa^{(g)},$$
wobei nun ersichtlich ist, dass nur noch $\kappa$ über die Gruppen hinweg variiert (wegen $^{(g)}$). Dies bedeutet also, dass unter Annahme der skalaren Invarianz Unterschiede in den Mittelwerte der Beobachtungen auf Unterschiede in den Mittelwerten der latenten Variablen zurückzuführen sind. Überlicherweise würde über die skalare Invarianz hinweg auch noch $\kappa$ über die Gruppen hinweg gleichgesetzt werden. Der Likelihood-Ratio-Test ($\chi^2$-Differenzentest) zwischen dem skalar-invarianten Modell und dem zusätzlich $\kappa$-invarianten Modell entscheidet dann darüber, ob die latente Variable sich im Mittel über die Gruppen hinweg unterscheidet. Wir schätzen das skalar-invariante Modell durch `group.equal = c("loadings", "intercepts")`. Da auch für die Mittelwertsstruktur Skalierer gesetzt werden müssen und diese üblicherweise auf den latenten Mittelwerten liegen ($\kappa=0$), muss dies bei der Invarianztestung berücksichtigt werden. Wenn wir die Skalierer auf den latenten Mittelwerten lassen und die Interzept der manifesten Variablen gleichsetzten, so nehmen wir damit auch implizit an, dass die latenten Mittelwerte über die Gruppen hinweg gleich sind. Um diesen Problem aus dem Weg zu gehen, stellt die `sem`-Funktion klugerweise ein, dass nur in der ersten Gruppe die latenten Mittelwerte/Interzepte ($\kappa$) auf 0 gesetzt werden, alle übrigen werden frei geschätzt. Dies schauen wir uns genauer an, wenn es weit ist!

Setzten wir zusätzlich zu Faktorladungen und Interzepte auch noch die Residualvarianzen ($\theta$) gleich über die Gruppen, so sprechen wir von **strikter Invarianz**. Gilt diese Invarianzbedingung, so lassen sich Unterschiede in den beobachteten Varianzen auf Unterschiede in den latenten Varianzen (also die Varianzen der latenten Variablen) zurückführen. Die lässt sich der Varianzformel entnehmen:
$$\mathbb{V}ar\left[X^{(g)}\right]=\lambda^2\phi^{(g)}+\theta.$$
Nur noch $\phi$ trägt auf der rechten Seite ein $^{(g)}$. Somit sind nur noch Unterschiede in $\phi$ verantwortlich für Unterschiede in $\mathbb{V}ar\left[X\right]$. Auch sind Unterschiede in den Reliabilitäten auf Unterschieden in den Varianzen der latenten Variablen zurückzuführen, denn die Formel für die Reliabilität ist:
$$\text{Rel}\left[X^{(g)}\right]=\frac{\lambda^2\phi^{(g)}}{\lambda^2\phi^{(g)}+\theta}=\frac{\lambda^2\phi^{(g)}}{\mathbb{V}ar\left[X^{(g)}\right]}.$$ 
Hier ist also auch nur noch $\phi$ variable über die Grupen und somit allein verantwortlich für Unterschiede in den Reliabilitäten über die Gruppen. Für standardisierte $X$ ($\mathbb{V}ar\left[X\right]=1$) und Skalierer auf der latenten Varianz ($\phi=1$) vereinfacht sicht diese Formel zu $\text{Rel}(X)=\lambda^2$, weswegen wir in diesem Fall die Reliabilität ganz einfach über die Faktorladungen erhalten. Dies gilt im Allgemeinen jedoch nicht! Wir schätzen das strikt-invariante Modell durch `group.equal = c("loadings", "intercepts", "residuals")`.

Gehen wir von **vollständiger Invarianz** aus, so sind alle Parameter über die Gruppen hinweg gleich. Es gibt also keinerlei Unterschiede mehr zwischen den Gruppen. Das vollständig-invariante Modell erhalten wir mit `group.equal = c("loadings", "intercepts", "residuals",` ` "means", "lv.variances", "lv.covariances", "regressions")`. Hierbei restringiert `"means"` die latenten Mittelwerte ($\kappa$), `"lv.variances"` die latenten (Residual-)Varianzen ($\phi$s und $\psi$s exogen und endogen) und `"regressions"` die Strukturpfadkoeffizienten ($\gamma$s und $\beta$s). Hätten wir Residualkovarianzen bei den manifesten oder latenten Variablen gehabt, so müssten wir diese auch mit `"residual.covariances"` sowie mit `"lv.covariances"` restringieren. Folglich lassen sich die Invarianzstufen nach der Anzahl der zu schätzenden Parameter oder den $df$ sortieren. Im konfigural-invarianten Modell müssen am meisten Parameter geschätzt werden, es besitzt also die kleinsten $df$, während im vollständig-invarianten Modell die wenigten Parameter geschätzt werden müssen, weswegen dieses auch die meisten $df$ hat. Die anderen Modell liegen wie folgt dazwischen (je weiter links, desto weniger $df$ und desto mehr Parameter sind zu schätzen):
$$\text{konfigural}\quad<\quad\text{metrisch/schwach}\quad<\quad\text{skalar/stark}\quad<\quad\text{strikt}\quad<\quad\text{vollständig}.$$

<mark style="background-color: orange"> Damit wir die `sem` Funktion nicht durch unsere vergebenen Labels verwirren und somit die Einstellungen mit `group.equal` überschreiben, **nehmen wir die Bezeichnungen für den indirekten Effekt heraus.** Wir nennen das Modell ohne Labels einfach `model_sem`. Für die aufwendigere Durchführung der MSA zu Fuß sowie simple grafische Repräsentationen siehe [Appendix B](#AppendixB). </mark>
...
<mark style="background-color: orange"> Dies bedeutet nun, dass Unterschiede in den Reliabilitäten tatsächlich auch auf Unterschiede in den Varianzen der latenten Variablen zurückzuführen sind. </mark>


#### Vollständige Invarianz

Unter vollständiger <mark style="background-color: orange"> oder struktureller </mark> Invarianz verstehen wir das Gleichsetzten aller Strukturparameter. Hier werden nun alle Varianzen, Residualvarianzen, ungerichteten und gerichteten Effekte des Strukturmodells über die Gruppen hinweg gleich gesetzt.
...
Wer davon noch nich genug hat, ist herzlich eingeladen im [Appendix C](#AppendixC) sich die mit der Invarianzidee verwandte Gleichsetzung der Faktorladungen innerhalb einer latenten Variable anzusehen: den Test auf essentielle $\tau$-Äquivalenz. 
